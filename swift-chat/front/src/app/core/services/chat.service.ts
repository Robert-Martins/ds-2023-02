...
export class ChatService {
  ...
  public connect(id: string): Promise<void> {
    return new Promise(
      () => {
        this.startClient();
        this.stompClient.connect({}, this.onConnected(id), this.onConnectionError);
      }
    );
  }
  ...
  private onConnected = (id: string): void => {
    ...
    this.events.next([]);
    this.subscription = this.stompClient.subscribe(`${this.CHAT_ROOM_TOPIC_PATH}/${id}`, this.onEventPublished);
  }
